...
namespace YGOCore.Game
{
    public class Player
    {
		...
		public void Testt()
        {
			...
			m_writer.Write((short)StocMessage.NETWORK_SERVER_ID);
			m_writer.Write((short)Program.Config.ClientVersion);
			m_writer.Write((short)7911);
			m_writer.Write((int)0);
			m_writer.Write((short)1); m_writer.Write((short)1); m_writer.Write((short)1); m_writer.Write((short)1); m_writer.Write((short)1);
			m_writer.Write((short)1); m_writer.Write((short)1); m_writer.Write((short)1); m_writer.Write((short)1); m_writer.Write((short)1);
			m_writer.Write((short)1); m_writer.Write((short)1); m_writer.Write((short)1); m_writer.Write((short)1); m_writer.Write((short)1);
			m_writer.Write((short)1); m_writer.Write((short)1); m_writer.Write((short)1); m_writer.Write((short)1); m_writer.Write((short)1);
			m_writer.Write((int)0);
			...
			m_writer.Write((bool)false);
			m_writer.Write((bool)false);
			m_writer.Write((bool)false);
			m_writer.Write((int)8000);
			m_writer.Write((byte)5);
			m_writer.Write((byte)1);
			m_writer.Write((short)120);
			IPEndPoint endPoint = new IPEndPoint(((IPEndPoint)m_client.m_client.Client.RemoteEndPoint).Address, 7921);
			Socket soc = new Socket(endPoint.Address.AddressFamily,
				SocketType.Dgram,
				ProtocolType.Udp);
			Logger.WriteLine(soc.SendTo(m_stream.ToArray(), endPoint));
			soc.SendTo(m_stream.ToArray(), endPoint);
			soc.Close();
        ...
        private void OnCreateGame(GameClientPacket packet)
        {
            ...
            room = GameManager.CreateOrGetGame(new GameConfig());
...
if (version >= Program.Config.ClientVersion)
			...
			else if (joinCommand.ToLower()=="s#" || joinCommand.ToLower()=="m#" || joinCommand.ToLower()=="t#")
			{
				room = GameManager.CreateOrGetGetRandomGame(joinCommand.ToLower());
			}
			else
			{
				room = GameManager.CreateRoom(new GameConfig(joinCommand));
				ServerMessage("你创建的房间名称为： "+room.Game.Config.Name);
			}	
			...
			string[] msg = Program.Config.ServerMessage.Split ('^');
        ...
        private void OnJoinGame(GameClientPacket packet)
        {
            ...
            string joinCommand = packet.ReadUnicode(256);
